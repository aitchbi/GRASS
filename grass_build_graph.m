...
function G = grass_build_graph(opts,overwrite)
...
if sts==1
    ...
    if res_o==res_i
        ...
        DoResample = true;
        if res_o>res_i % 3D, sample by sample
            d = 'approach1';
        else           % 2D, slice by slice
            d = 'approach2';
        end
        fprintf('\n..Downsampling mask.. ');
    end
    ...
    DoResample = true;
    d = 'approach1';
    fprintf('\n..Downsampling mask.. [non-iso voxels -> isotropic voxels]');
end
if DoResample
    interp = 1;
    f_tmp = hb_resample_vol_april2022(f_tmp,res_o,interp,f_tmp,d);
end
...
if 0
    ...
    d1 = ml_batch_gifti(d1,h_mask.mat);
else
    d = struct;
    [d.vertices,d.faces] = read_surf(strrep(G.f.surface.pial,'.gii',''));
end
